...
public class TileMapEditor : Editor {
	...
	void OnSceneGUI() {
		if (brush != null) {
			...
			if (board.texture2D != null && mouseOnMap) {
				...
				if (cEvent.shift)
					Draw ();
				if (cEvent.type==EventType.KeyDown && cEvent.keyCode==KeyCode.LeftAlt)
					RemoveTile ();
				...
				if (cEvent.type == EventType.KeyDown && cEvent.keyCode == KeyCode.LeftControl)
				if (targetTile == null) {
				...
				if (cEvent.type == EventType.KeyUp && cEvent.keyCode == KeyCode.LeftControl) {
					brush.renderer2D.sortingOrder = 1000;
					if (targetTile != null) {
						targetTile = null;
						dragTile = false;
	...
	void ToggleStar() {
		...
		if (tile != null && !tile.GetComponent<Tile> ().isSolid) {
			...
			UnityEngine.Object prefab = AssetDatabase.LoadAssetAtPath ("Assets/Prefabs/star.prefab", typeof(GameObject));
