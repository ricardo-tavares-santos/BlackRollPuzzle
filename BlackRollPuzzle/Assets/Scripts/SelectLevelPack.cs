...
public class SelectLevelPack : MonoBehaviour
{
    ...
    void FillGrid()
    {
        ...
        for (int i = 0; i < GameManager.instance.maxPacks; i++)
        {
            ...
            if (GameConfig.instance.unlockAllLevelForTesting) packUnlocked = 1;

            if (packUnlocked == 1)
            {
                buttonUnlocked.SetActive(true);
                GameObject text = buttonUnlocked.transform.Find("StarsCollectedText").gameObject;
                text.GetComponent<Text>().text = starsCollected + "/40";
